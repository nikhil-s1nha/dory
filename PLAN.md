...
| M5 | Smart-stack priority & advancement | 🟡 **logic complete, render blocked** — pure selection/advancement (priority photo>drawing>music, per-open cycling), 15 tests; cursor persistence + per-open delivery to the App Group **verified on device**. But the placed widget is frozen on one snapshot and won't repaint — open bug, see [WIDGET-FREEZE.md](./WIDGET-FREEZE.md) |
